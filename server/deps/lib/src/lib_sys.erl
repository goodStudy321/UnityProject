...
process_abn(MsgLen) ->
    L = lists:foldl(
        fun(P,Acc) ->
            case catch erlang:process_info(P, message_queue_len) of
    ...
    [begin
         io:format("~-30s ~-20w ~-32w Len:~-10w Red:~-15w THeap:~-8w Heap:~-8w Stack:~-8w~n",[PName, P, Fun, Len, Red, THeap, Heap, Stack])
    ...
    lists:foreach(
        fun(Node) ->
            lists:foreach(
                ...
                end, rpc:call(Node, lib_sys, get_memory_pids, [Memory]))
        end, [node() | nodes()]),
    ok.
...
%% @doc 打印占用内存超过1MB的进程信息
-spec get_memory() -> ok.
get_memory() ->
    io:fwrite("process count:~p~n~p value is large than ~p count:~p~nLists:~p~n",
        get_process_info_and_large_than_value(memory, 1024 * 1024) ),
    ok.

%% @doc 打印占用内存超过一定MB值的进程信息
-spec get_memory(integer()) -> ok.
get_memory(Value) ->
    io:fwrite("process count:~p~n~p value is large than ~p count:~p~nLists:~p~n",
        get_process_info_and_large_than_value(memory, 1024 * 1024 * Value) ),
    ok.
