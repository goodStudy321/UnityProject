...
proc_attrs(binary_memory, Pid) ->
    case process_info(Pid, [binary, registered_name,
                            current_function, current_stacktrace, initial_call]) of
        [{_, Bins}, {registered_name,Name}, Init, Cur, CurStack] ->
            {ok, {Pid, binary_memory(Bins), [Name || is_atom(Name)]++[Init, Cur, CurStack]}};
        undefined ->
            {error, undefined}
    end;
proc_attrs(AttrName, Pid) ->
    case process_info(Pid, [AttrName, registered_name,
                            current_function, current_stacktrace, initial_call]) of
        [{_, Attr}, {registered_name,Name}, Init, Cur, CurStack] ->
            {ok, {Pid, Attr, [Name || is_atom(Name)]++[Init, Cur, CurStack]}};
        undefined ->
            {error, undefined}
    end.
