...
-record(map_config, {map_id, weight, height}).
...
bstar_search(MapConfig, StartX, StartY, EndX, EndY) ->
    ...
    wipe_angle(start_search(BStarMap2)).
...
start_search(#bstar_map{open_list = [CurrentCell|Rest], target = Target} = BStarMap) ->
    ...
    %% 向着目标前进
        ...
        true ->
            ...
            Dir3 = ?IF(Dir2 >= ?MAX_DIRECTION, Dir2 - ?MAX_DIRECTION, Dir2),
...
get_branch([Count|Rest], CurrentCell, BranchDir, Dir, #bstar_map{target = Target, map_config = #map_config{weight = Weight, height = Height}} = BStarMap) ->
    Dir2 = ?IF(Dir >= ?MAX_DIRECTION, Dir - ?MAX_DIRECTION, Dir),
...
%% 取得格子
get_cell(#bstar_map{cell_dict = Dict}, X, Y) ->
    case dict:find({X, Y}, Dict) of
        {ok, O} -> O;
        _ ->
             CellState = ?IF(map_base_data:is_exist(X, Y), ?cell_state_none, ?cell_state_balk),
...
%% 取得周围方向
around(0) -> #pos{x = 0, y = 1};   %% 向右移动一格
around(1) -> #pos{x = -1, y = 0};   %% 向下移动一格
around(2) -> #pos{x = 0, y = -1};   %% 向左移动一格
around(3) -> #pos{x = 1, y = 0}.    %% 向上移动一格
...
g_GetDirDiff(OriginX, OriginY, TargetX, TargetY, X, Y) ->
    ...
    DirDiff2 = ?IF(DirDiff < 0, DirDiff + 64, DirDiff),
    ?IF(DirDiff2 > 32, 64 - DirDiff2, DirDiff2).
...
get_branchAround([Count|Rest], CurrentCell, Branch, TestDir, #bstar_map{map_config = #map_config{weight = Weight, height = Height}} = BStarMap) ->
    TestDir2 = ?IF(TestDir =:= ?MAX_DIRECTION, 0, TestDir),
...
wipe_angle(CeilList) ->
    [Cell|_] = CeilList,
    wipe_angle2(CeilList, [Cell]).
wipe_angle2([], ResCeil) ->
    lists:reverse(ResCeil);
wipe_angle2(List, ResCeil) when length(List) == 1 ->
    [Cell1] = List,
    wipe_angle2([], [Cell1|ResCeil]);
wipe_angle2(List, ResCeil) when length(List) == 2 ->
    [Cell1, Cell2] = List,
    ResCeil1 = [Cell1|ResCeil],
    wipe_angle2([], [Cell2|ResCeil1]);
wipe_angle2([#cell{x = X, y = Y}, Cell1, #cell{x = X2, y = Y2} = Cell2|List], ResCeil) ->
    CX = X - X2,
    CY = Y - Y2,
    case (CX == 1 orelse CX == -1) andalso (CY == 1 orelse CY == -1) of
        true ->
            wipe_angle2([Cell2|List], [Cell2|ResCeil]);
        false ->
            List2 = [Cell2|List],
            wipe_angle2([Cell1|List2], [Cell1|ResCeil])
    end.
...
test() ->
    ...
    [{Weight, Height}] = map_base_data:info(MapID),
    MapConfig = #map_config{weight = Weight, height = Height},
    lib_profile:tc_begin(),
    List = bstar_search(MapConfig, 105, 16, 185, 98),
    {Weight, Height, lib_profile:tc_end(), List}.
astar_test() ->
    lib_profile:tc_begin(),
    map_base_data:init(90002),
    Pos1 = #r_pos{tx=105, ty=16},
    Pos2 = #r_pos{tx=185, ty=98},
    lib_profile:tc_begin(),
    List = mod_astar_pathfinding:find_path(Pos1, Pos2, 500),
    {lib_profile:tc_end(), List}.
test(Max) ->
    MapID = 10001,
    map_base_data:init(MapID),
    [{Weight, Height}] = map_base_data:info(MapID),
    MapConfig = #map_config{weight = Weight, height = Height},
    test(MapConfig, Max, 361,155, 163,147).
