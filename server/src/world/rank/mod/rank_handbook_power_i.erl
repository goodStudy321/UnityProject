...
%%%
%%% @end
%%% Created : 18. 三月 2019 10:37
%%%-------------------------------------------------------------------
-module(rank_handbook_power_i).
-author("WZP").
-include("global.hrl").
-include("rank.hrl").
-include("act.hrl").
...
init_rank(RankID, MaxNum) ->
    ...
    Ranks2 = [{RoleID, RankInfo} || #r_rank_handbook_power{role_id = RoleID} = RankInfo <- Ranks],
    lib_minheap:new_heap(RankID, MaxNum, {?MODULE, cmp_rank}, Ranks2).

cmp_rank(RankInfo1, RankInfo2) ->
    #r_rank_handbook_power{power = Level1, update_time = UpdateTime1} = RankInfo1,
    #r_rank_handbook_power{power = Level2, update_time = UpdateTime2} = RankInfo2,
    rank_misc:cmp([{Level1, Level2}, {UpdateTime2, UpdateTime1}]).

rank(RankID) ->
    case world_act_server:is_act_open(?ACT_OSS_HANDBOOK) of
        true ->
            AllEs = lib_minheap:get_all_elements(RankID),
            SortAllEs = lists:sort(fun cmp_rank/2, AllEs),
            {SortAllEs1, _} = lists:foldl(
                fun(E, {Acc, Rank}) ->
                    E1 = E#r_rank_handbook_power{rank = Rank},
                    {[E1|Acc], Rank + 1}
