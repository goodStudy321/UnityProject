...
init_rank(RankID, MaxNum) ->
    ...
    Ranks2 = [{RoleID, RankInfo} || #r_rank_god_weapon_power{role_id = RoleID} = RankInfo <- Ranks],
    lib_minheap:new_heap(RankID, MaxNum, {?MODULE, cmp_rank}, Ranks2).

cmp_rank(RankInfo1, RankInfo2) ->
    #r_rank_god_weapon_power{god_weapon_power = Power1, update_time = UpdateTime1} = RankInfo1,
    #r_rank_god_weapon_power{god_weapon_power = Power2, update_time = UpdateTime2} = RankInfo2,
...
rank(RankID) ->
    ...
    {SortAllEs1, _} = lists:foldl(
        fun(E, {Acc, Rank}) ->
            E1 = E#r_rank_god_weapon_power{rank = Rank},
            {[E1 | Acc], Rank + 1}
        end, {[], 1}, lists:reverse(SortAllEs)),
    db:insert(?DB_RANK_P, #r_rank{rank_id = RankID, ranks = SortAllEs1}).

trans_to_r_rank(List) ->
    [{RoleID, #r_rank_god_weapon_power{
        role_id = RoleID,
        god_weapon_level = Level,
        god_weapon_power = Power,
...
trans_to_p_rank2(RoleRank) ->
    #r_rank_god_weapon_power{rank = Rank, role_id = RoleID, god_weapon_level = GodWeaponLevel, god_weapon_power = Power} = RoleRank,
    ...
    #p_rank{
        ...
        kv_list = [
            #p_dkv{id = ?KEY_GOD_WEAPON_LEVEL, val = GodWeaponLevel},
...
trans_to_log([E | SortAllEs], Logs) ->
    #r_rank_god_weapon_power{rank = Rank, role_id = RoleID, god_weapon_level = GodWeaponLevel, god_weapon_power = GodWeaponPower} = E,
    ...
    Log = #log_rank{role_id = RoleID, role_name = unicode:characters_to_binary(RoleName), category = Category, role_vip_level = common_role_data:get_role_vip_level(RoleID), rank_type = ?RANK_GOD_WEAPON_POWER,
        rank_value = GodWeaponPower, rank_value2 = GodWeaponLevel,
