...
init_rank(RankID, MaxNum) ->
    ...
    Ranks2 = [{RoleID, RankInfo} || #r_rank_wing_power{role_id = RoleID} = RankInfo <- Ranks],
    lib_minheap:new_heap(RankID, MaxNum, {?MODULE, cmp_rank}, Ranks2).

cmp_rank(RankInfo1, RankInfo2) ->
    #r_rank_wing_power{wing_power = Power1, update_time = UpdateTime1} = RankInfo1,
    #r_rank_wing_power{wing_power = Power2, update_time = UpdateTime2} = RankInfo2,
...
trans_to_p_rank2(RoleRank) ->
    #r_rank_wing_power{rank = Rank, role_id = RoleID, wing_level = WingLevel, wing_power = Power} = RoleRank,
    ...
    #p_rank{
        ...
        kv_list = [
            #p_dkv{id = ?KEY_WING_LEVEL, val = WingLevel},
...
trans_to_log([E | SortAllEs], Logs) ->
    #r_rank_wing_power{rank = Rank, role_id = RoleID, wing_level = WingLevel, wing_power = WingPower} = E,
    ...
    Log = #log_rank{role_id = RoleID, role_name = unicode:characters_to_binary(RoleName), category = Category, role_vip_level = common_role_data:get_role_vip_level(RoleID), rank_type = ?RANK_WING_POWER,
        rank_value = WingPower, rank_value2 = WingLevel,
