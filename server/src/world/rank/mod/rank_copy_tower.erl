...
init_rank(RankID, MaxNum) ->
    ...
    Ranks2 = [{RoleID, RankInfo} || #r_rank_copy_tower{role_id = RoleID} = RankInfo <- Ranks],
    lib_minheap:new_heap(RankID, MaxNum, {?MODULE, cmp_rank}, Ranks2).

cmp_rank(RankInfo1, RankInfo2) ->
    #r_rank_copy_tower{tower = Tower1, update_time = UpdateTime1} = RankInfo1,
    #r_rank_copy_tower{tower = Tower2,update_time = UpdateTime2} = RankInfo2,
    rank_misc:cmp([{Tower1, Tower2}, {UpdateTime2, UpdateTime1}]).
...
rank(RankID) ->
    ...
    {SortAllEs1, _} = lists:foldl(
        fun(E, {Acc, Rank}) ->
            E1 = E#r_rank_copy_tower{rank = Rank},
            {[E1 | Acc], Rank + 1}
...
trans_to_r_rank(List) ->
    [{RoleID, #r_rank_copy_tower{role_id = RoleID, tower = Tower, update_time = Time}} || {RoleID, Tower, Time} <- List].

trans_to_p_rank(List) ->
    [trans_to_p_rank2(Rank) || Rank <- List].

trans_to_p_rank2(RoleRank) ->
    #r_rank_copy_tower{rank = Rank, role_id = RoleID, tower = Tower} = RoleRank,
    ...
    #p_rank{
        ...
        kv_list = [#p_dkv{id = ?KEY_TOWER, val = Tower}, #p_dkv{id = ?KEY_POWER, val = Power}]
    }.
...
trans_to_log([E|SortAllEs], Logs) ->
    #r_rank_copy_tower{rank = Rank, role_id = RoleID, tower = Tower} = E,
    ...
    Log = #log_rank{role_id = RoleID, role_name = unicode:characters_to_binary(RoleName), category = Category,
                    role_vip_level = common_role_data:get_role_vip_level(RoleID), rank_type = ?RANK_COPY_TOWER, rank_value = Tower,
