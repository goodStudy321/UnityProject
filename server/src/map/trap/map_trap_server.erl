...
monster_i(MapID, TrapID) ->
    pname_server:call(get_name(map_misc:get_map_pname(MapID, map_branch_manager:get_map_cur_extra_id(MapID))), {trap_i, TrapID}).
monster_i(MapID, ExtraID, TrapID) ->
    pname_server:call(get_name(map_misc:get_map_pname(MapID, ExtraID)), {trap_i, TrapID}).
...
get_name(MapPName) ->
    lib_tool:list_to_atom(lists:concat([lib_tool:to_list(MapPName),"_trap"])).
...
handle_info({'EXIT', PID, _Reason}, State) ->
    ?ERROR_MSG("map trap server receive exit msg from ~p, resaon: ~p", [PID, _Reason]),
...
%%% Internal functions
%%%===================================================================
do_handle({loop_msec, LoopMs}) ->
    time_tool:now_ms_cached(LoopMs),
    mod_trap:loop_ms(LoopMs);
%%对指定的模块发送消息,通用,建议使用
...
do_trap_i(TrapID) ->
    mod_trap_data:get_trap_data(TrapID).

do_key_i(Key) ->
    erlang:get({mod_trap_data, Key}).