...
class Channel extends Main {
    ...
    public function channelGameData(){
        ...
        $res = $model->alias('g')
            ->join('channel c','g.channel_id = c.channel_id', "LEFT")
        ...
        if(!empty($data)){
            foreach($data as &$v){
                ...
                $v['action'] .= '<button class="layui-btn layui-btn-xs " onclick="x_admin_show(\'修改包渠道\',\'edit.html?id='.$v['id'].'\',500,450)">修改包渠道</i></button>';
            }
            unset($v);
        }
    ...
    public function add(){
        if($this->request->isAjax()){
            ...
            $data = [
                ...
                'game_channel_id' => (int)$post['game_channel_id'],
                'name'            => trim($post['name']),
                'add_time'        => time(),
            ];
            if(in_array('', $data))
    ...
    public function channelData(){
        ...
        if(!empty($data)){
            foreach($data as &$v){
                $v['action'] = '<button onclick="del('.$v['id'].')" class="layui-btn layui-btn-xs layui-btn-danger">删除</button>';
                $v['action'] .= '<button class="layui-btn layui-btn-xs " onclick="x_admin_show(\'修改渠道\',\'channelEdit.html?id='.$v['id'].'\',500,300)">修改渠道</i></button>';
            }
            unset($v);
        }
        return [
    ...
    public function getGameChannelByChannelid(){
        $channel_id = $this->request->post('channel_id', 'int', 0);

        $game_channel = (new ChannelGame())->field('channel_id,game_channel_id,name')->where("channel_id=$channel_id")->select();
        $game_channel = $game_channel ? $game_channel : [];

        $this->returnJson(10200, '', $game_channel);
    }
