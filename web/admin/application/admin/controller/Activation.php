...
class Activation extends Main {
    ...
    public function examine(){
        ...
        $this->assign('id', $id);
        $this->assign('status', $this->code_status);
        return $this->fetch();
    }
    ...
    public function examine_data($limit = 0, $page = 0){
        ...
        foreach ($res as $key=>&$val){
            ...
            $val['code_status']  = $this->code_status[$val['code_status']-1];
    ...
    public function user(){
        $server = DB::name('server')->field('server_id,server_name')->where(['is_verify'=>1])->select();
        $this->assign('server', $server);
        $this->assign('code_status', $this->code_status);
        return $this->fetch();
    ...
    public function user_data(){
        ...
        if(!empty($parm['find'])){
            ...
            if(!empty($parm['server_id'])&&$parm['server_id']!='all')
                $map['l.server_id'] = (int)$parm['server_id']; //服务器
