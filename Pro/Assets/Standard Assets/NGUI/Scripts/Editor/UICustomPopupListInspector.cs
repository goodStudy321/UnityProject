...
[CustomEditor(typeof(UICustomPopupList), true)]
public class UICustomPopupListInspector : UIWidgetContainerEditor
{
	enum FontType
	{
		Bitmap,
	...
	void OnEnable ()
	{
		...
		mList = target as UICustomPopupList;
		...
		if (mList.atlas == null && mList.background2DSprite == null && mList.highlight2DSprite == null)
		{
            ...
            mList.lblbgSprite = NGUISettings.selectedSprite;
            EditorUtility.SetDirty(mList);
		}
	}
    ...
    void OnLblBG(string spriteName)
    {
        RegisterUndo();
        mList.lblbgSprite = spriteName;
        Repaint();
    }
	...
	void DrawAtlas()
	{
		if (NGUIEditorTools.DrawHeader("Atlas"))
		{
			...
			if (atlasSp != null && atlasSp.objectReferenceValue != null)
			{
                ...
                NGUIEditorTools.DrawPaddedSpriteField("Default", mList.atlas, mList.lblbgSprite, OnLblBG);
            }
			else
			{
            ...
            DrawDimensions(serializedObject);
    ...
    static void DrawDimensions(SerializedObject so)
    {
        GUILayout.BeginHorizontal();
        {

            GUI.changed = false;
            int valw = EditorGUILayout.IntField("Size", so.FindProperty("CustomW").intValue, GUILayout.MinWidth(100f));

            if (GUI.changed)
            {
                so.FindProperty("CustomW").intValue = valw;
            }

            NGUIEditorTools.SetLabelWidth(12f);

            GUI.changed = false;
            int valh = EditorGUILayout.IntField("x", so.FindProperty("CustomH").intValue, GUILayout.MinWidth(30f));

            if (GUI.changed)
            {
                so.FindProperty("CustomH").intValue = valh;
            }
        ...
        NGUIEditorTools.DrawProperty("X", so, "SpacingX", GUILayout.MinWidth(40f));
        NGUIEditorTools.DrawProperty("Y", so, "SpacingY", GUILayout.MinWidth(40f));
