...
public class UILabel : UIWidget
{
	...
	{
        string text2;
        NGUIText.WrapText(processedText, out text2);
        //string[] lines = text2.Split('\n');
        //Debug.Log("lines = " + lines.Length);

        //string s = printedText;
        //string calS = processedText;
        string s = processedText;
        //Debug.Log("xxxxxxxxxxxxxxxxx    s.len: " + s.Length + "   calS: " + calS.Length);
        ...
        if (characterIndex != -1 && characterIndex < s.Length)
        {
            ...
            {
                string retStr = s.Substring(linkStart, linkEnd - linkStart);
    ...
    /// LY add begin ///
    public Vector2 CalculatePrintedSize(string calText)
    {
        Vector2 retSize = Vector2.zero;
        if(string.IsNullOrEmpty(calText) == true)
        {
            return retSize;
        }
        ...
        int tFinalFontSize = Mathf.Abs(defaultFontSize);
        float tScale = 1f;

        string tProcessedText = "";
        if (NGUIText.regionWidth < 1 || NGUIText.regionHeight < 0)
        {
            tProcessedText = "";
            return retSize;
        ...
        if (tFinalFontSize > 0)
        {
            ...
            for (int ps = tFinalFontSize; ps > 0; --ps)
            {
                // Adjust either the size, or the scale
                if (adjustSize)
                {
                    tFinalFontSize = ps;
                    NGUIText.fontSize = tFinalFontSize;
                }
                else
                {
                    tScale = (float)ps / tFinalFontSize;
                    NGUIText.fontScale = isDynamic ? tScale : ((float)mFontSize / mFont.defaultSize) * tScale;
                }

                NGUIText.Update(false);

                // Wrap the text
                bool fits = NGUIText.WrapText(calText, out tProcessedText, false, false, mOverflowEllipsis);
                ...
                {
                    retSize = NGUIText.CalculatePrintedSize(tProcessedText);

                    int w = Mathf.Max(minWidth, Mathf.RoundToInt(retSize.x));
                    if (regionX != 1f) w = Mathf.RoundToInt(w / regionX);
                    int h = Mathf.Max(minHeight, Mathf.RoundToInt(retSize.y));
                    if (regionY != 1f) h = Mathf.RoundToInt(h / regionY);
