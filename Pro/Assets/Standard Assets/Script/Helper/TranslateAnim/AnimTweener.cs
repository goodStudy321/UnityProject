...
public abstract class AnimTweener : MonoBehaviour
{
	...
	/// </summary>
	static public AnimTweener current;
	...
	/// </summary>
	[HideInInspector]
	public AnimType style = AnimType.AT_ONCE;
	...
	protected void Update () { DoUpdate(); }
	...
	protected void DoUpdate ()
	{
		...
		else if (style == AnimType.AT_PINGPONG)
		{
		...
		if ((style == AnimType.AT_ONCE) && (duration == 0f || mFactor > 1f || mFactor < 0f))
		{
	...
	/// </summary>
	static public T Begin<T> (GameObject go, float duration, float delay = 0f) where T : AnimTweener
	{
		...
		if (comp == null)
		{
			...
			if (comp == null)
			{
				Debug.LogError("Unable to add " + typeof(T) + " to " + AnimTweenerTool.GetHierarchy(go), go);
				return null;
			}
		}
