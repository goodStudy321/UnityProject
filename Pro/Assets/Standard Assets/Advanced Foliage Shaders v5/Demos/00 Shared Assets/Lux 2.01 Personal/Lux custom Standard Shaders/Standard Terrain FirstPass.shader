Shader "Lux/Standard Lighting/Nature/Terrain/Standard" {
	...
	SubShader {
		...
		CGPROGRAM
		#pragma surface surf LuxStandardSpecular vertex:SplatmapVert finalcolor:SplatmapFinalColor finalgbuffer:SplatmapFinalGBuffer fullforwardshadows noambient nodynlightmap
		...
		void surf (Input IN, inout SurfaceOutputLuxStandardSpecular o) {
			...
			LUX_SET_HEIGHT( Luminance(o.Albedo) )
			...
			//	////////////////////////////////////
			//	Add per pixel ambient lighting
			//	EnergyConservationBetweenDiffuseAndSpecular
			half3 eAlbedo = 0.0;
			#if UNITY_CONSERVE_ENERGY_MONOCHROME
				eAlbedo = o.Albedo * (1.0 - SpecularStrength(o.Specular));
			#else
				eAlbedo = o.Albedo * (half3(1.0, 1.0, 1.0) - specColor);
			#endif
			//	Completely per-pixel
			half3 ambient = ShadeSH9( half4(WorldNormalVector(IN, o.Normal), 1.0));
			ambient = max(half3(0.0, 0.0, 0.0), ambient);
			o.Emission = eAlbedo * ambient * o.Occlusion;
