...
#pragma warning restore
            ...
            /// LY add begin ///
            AudioSampler.playInEditor = false;
            /// LY add end ///
		}
		...
		public void Play(Cutscene.WrapMode wrapMode = Cutscene.WrapMode.Loop, System.Action callback = null){
            ...
            /// LY add begin ///
            AudioSampler.playInEditor = true;
            /// LY add end ///
		}
		...
		class ActionClipWrapper{
			...
			void DoClipContextMenu(){
                ...
                // LY add begin //
                if(action is ActionClips.OverlayText)
                {
                    ActionClips.OverlayText tOT = action as ActionClips.OverlayText;
                    string tText = tOT.text;
                    EaseType tET = tOT.interpolation;
                    Color tCol = tOT.color;
                    float tBI = tOT.blendIn;
                    float tBO = tOT.blendOut;
                    float tIn = tOT.startTime;
                    float tOut = tOT.endTime;

                    menu.AddItem(new GUIContent("Change to Captions"), false, () =>
                    {
                        (action.parent as CutsceneTrack).DeleteAction(action);
                        ActionClips.Captions tCT = (action.parent as CutsceneTrack).AddAction(typeof(ActionClips.Captions), 0f) as ActionClips.Captions;
                        tCT.text = tText;
                        tCT.interpolation = tET;
                        tCT.color = tCol;
                        tCT.blendIn = tBI;
                        tCT.blendOut = tBO;
                        tCT.startTime = tIn;
