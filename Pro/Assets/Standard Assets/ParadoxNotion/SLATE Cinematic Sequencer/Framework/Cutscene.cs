...
namespace Slate{
    ...
    public class GroupActor
    {
        ...
        /// 替代类型
        /// </summary>
        public ushort mReplaceType;
        /// <summary>
        /// 新主体物体
        /// </summary>
        public GameObject mNewActor = null;
        /// <summary>
        /// 动画片段资源
        /// </summary>
        public List<AnimationClip> mACList = null;
        ...
        /// 音频片段（音频轨迹专用）
        /// </summary>
        public List<AudioClip> mSCList = null;
    };
	...
	public class Cutscene : MonoBehaviour, IDirector{
        ...
        public void Play() { Play(0, null, null); }
        public void Play(List<GroupActor> ga = null, string camPath = null) { Play(0, ga, camPath); }
		public void Play(System.Action callback, List<GroupActor> ga = null, string camPath = null) { Play(0, callback, ga, camPath); }
		public void Play(float startTime, List<GroupActor> ga = null, string camPath = null)
        {
            Play(startTime, length, defaultWrapMode, null, PlayingDirection.Forwards, ga, camPath);
        }
		public void Play(float startTime, System.Action callback, List<GroupActor> ga = null, string camPath = null)
        {
            Play(startTime, length, defaultWrapMode, callback, PlayingDirection.Forwards, ga, camPath);
        }
		public void Play
			(
			float startTime,
			...
			)
		{
            ...
            {
                DirectorPropertiesTrack tTrack = null;
                // 获取导演参数轨道
                for (int a = 0; a < directorGroup.tracks.Count; a++)
                {
                    if (directorGroup.tracks[a] is DirectorPropertiesTrack)
                    {
                        tTrack = directorGroup.tracks[a] as DirectorPropertiesTrack;
                        break;
                    }
                }
                // 替换子路径
                if (tTrack != null)
                {
                    if (tTrack.animationData != null || tTrack.animationData.animatedParameters != null)
                    {
                        List<AnimatedParameter> tAPList = tTrack.animationData.animatedParameters;
                        for (int a = 0; a < tAPList.Count; a++)
                        {
                            AnimatedParameter tAP = tAPList[a];
                            if (tAP != null && tAP.transformHierarchyPath == "Render Camera")
                            {
                                tAP.transformHierarchyPath = camPath; // "Root<Camera>"
                            }
                        }
                    }
                }
            ...
            //            if(tAP != null && tAP.transformHierarchyPath == "Render Camera")
            //            {
            //                tAP.transformHierarchyPath = "Root<Camera>";
            //            }
            //        }
            //    }
            //}
            ...
            {
                for(int a = 0; a < ga.Count; a++)
                {
                    if(ga[a].mReplaceType == 1)
                    {
                        if (ga[a].mNewActor == null)
                        {
                            Debug.LogWarning("LY, Cutscene change actor is null !!!");
                            //CutscenePlayMgr.instance.SkipCutscene();
                            EventMgr.Trigger("EventSkipCutScene");
                            return;
                        }
                        for (int b = 0; b < ga[a].mACList.Count; b++)
                        {
                            if (ga[a].mACList[b] == null)
                            {
                                Debug.LogWarning("LY, Cutscene change AnimationClip is null !!!");
                                //CutscenePlayMgr.instance.SkipCutscene();
                                EventMgr.Trigger("EventSkipCutScene");
                    ...
                    else if(ga[a].mReplaceType == 2)
                    {
                        for (int b = 0; b < ga[a].mSCList.Count; b++)
                        {
                            if (ga[a].mSCList[b] == null)
                            {
                                Debug.LogWarning("LY, Cutscene change AudioClip is null !!!");
                                //CutscenePlayMgr.instance.SkipCutscene();
                                EventMgr.Trigger("EventSkipCutScene");
...
#region UNITY GUI
        ...
        /// </summary>
        /// <param name="ga"></param>
        public void ChangeGroupActor(GroupActor ga)
        {
            if(ga.mReplaceType == 1)
            {
                ChangeGroupActor(ga.mGroupName, ga.mNewActor, ga.mACList);
            }
            else if(ga.mReplaceType == 2)
            {
                ChangeAudioTrack(ga.mGroupName, ga.mTrackName, ga.mSCList);
            }
        }
        ...
        /// 替换动画组角色
        /// </summary>
        /// <param name="gaList"></param>
        public void ChangeGroupActor(List<GroupActor> gaList)
        {
            for(int a = 0; a < gaList.Count; a++)
            {
                ChangeGroupActor(gaList[a]);
            }

            if (groups != null && groups.Count > 0)
            {
                for (int a = groups.Count - 1; a >= 0; a--)
                {
                    if (groups[a].actor == null || groups[a].gameObject == null)
                    {
                        groups.RemoveAt(a);
                    }
                }
        ...
        protected void ChangeGroupActor(string groupName, GameObject newActor, List<AnimationClip> clips = null)
        {
            for(int a = 0; a < groups.Count; a++)
            {
                if(groups[a].name == groupName)
                {
                    if(groups[a].actor != null && groups[a].actor != groups[a].oriActor)
                    {
                        groups[a].actor.transform.parent = null;
                        GameObject.DestroyImmediate(groups[a].actor);
                    }
                    groups[a].actor = newActor;

                    for(int b = 0; b < groups[a].tracks.Count; b++)
                    {
                        if(groups[a].tracks[b] is AnimationTrack)
                        {
                            ((AnimationTrack)groups[a].tracks[b]).ChangeClipRes(clips);
                        }
                    }
                    break;
                }
        ...
        /// <param name="groupName"></param>
        /// <param name="newActor"></param>
        protected void ChangeAudioTrack(string groupName, string trackName, List<AudioClip> clips = null)
        {
            for (int a = 0; a < groups.Count; a++)
            {
                if (groups[a].name == groupName)
                {
                    for (int b = 0; b < groups[a].tracks.Count; b++)
                    {
                        if (groups[a].tracks[b] is AudioTrack && groups[a].tracks[b].name == trackName)
                        {
                            AudioTrack cTrack = (AudioTrack)groups[a].tracks[b];
                            for(int c = 0; c < cTrack.actions.Count; c ++)
                            {
                                ((ActionClips.PlayAudio)cTrack.actions[c]).audioClip = clips[c];
                            }
                            break;
                        }
                    }
