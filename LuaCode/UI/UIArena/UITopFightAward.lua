--[[
 	authors 	:Liu
 	date    	:2018-11-29 11:00:00
 	descrition 	:青云之巅排行奖励
--]]

UITopFightAward = Super:New{Name = "UITopFightAward"}

local My = UITopFightAward

require("UI/UIArena/UITopFightAwardIt")

function My:Init(root)
    local des = self.Name
    local SetB = UITool.SetBtnClick
    local str = "ScrollView/Grid"
    local gridTran = TransTool.Find(root, str, des)
    local aItem = TransTool.FindChild(root, str.."/rankItem", des)

    SetB(root, "closeBtn", des, self.OnClose, self)
    self.go = root.gameObject
    self.itList = {}
    self:InitItem(aItem, gridTran)
end

--初始化奖励项
function My:InitItem(aItem, gridTran)
    local Add = TransTool.AddChild
    local list = {}
    for k,v in pairs(TopFRankCfg) do
        table.insert(list, v)
    end
    table.sort(list, function(a,b) return a.id < b.id end)
    for i,v in ipairs(list) do
        local item = Instantiate(aItem)
        local tran = item.transform
        Add(gridTran, tran)
        local it = ObjPool.Get(UITopFightAwardIt)
        it:Init(tran, v)
        table.insert(self.itList, it)
    end
    aItem:SetActive(false)
end

--点击关闭按钮
function My:OnClose()
    self:UpShow(false)
end

--更新显示
function My:UpShow(state)
    self.go:SetActive(state)
end

--清理缓存
function My:Clear()
	
end
    
--释放资源
function My:Dispose()
    self:Clear()
    ListTool.ClearToPool(self.itList)
end

return My