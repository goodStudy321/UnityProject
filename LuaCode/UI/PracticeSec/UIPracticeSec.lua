require("UI/PracticeSec/Prac/PracLv")
require("UI/PracticeSec/Prac/PracReward")
require("UI/PracticeSec/Prac/PracMission")

UIPracticeSec = UIBase:New{Name = "UIPracticeSec"}
local My = UIPracticeSec
...
function My:InitCustom()
    ...
    self.pracLvInfo = ObjPool.Get(PracLv)
    self.pracLvInfo:Init(Find(root,"PracLv",des))

    if PracSecMgr:IsOpen() then
        self.strDic["1"] = "奖励"
        local module1 = Find(root, "PracRwd", des)
        self:SetModInfo(module1, PracReward, 1)

        self.strDic["2"] = "任务"
        local module2 = Find(root, "PracMission", des)
        self:SetModInfo(module2, PracMission, 2)
    end
...
function My:SetLnsr(func)
    PracticeSecMgr.eUpAction[func](PracticeSecMgr.eUpAction, self.UpAction, self)
    PropMgr.eGetAdd[func](PropMgr.eGetAdd, self.OnAdd, self)
end

-- 道具添加
function My:OnAdd(action,dic)
	if action==10439 or action == 10440 then		
		self.dic=dic
...
function My:UpAction()
    ...
    for k,v in pairs(PracticeSecMgr.actionDic) do
        local go = dic[k]
        if go then
            go:SetActive(v)
        end
...
function My:DisposeCustom()
    if self.pracLvInfo then
        ObjPool.Add(self.pracLvInfo)
        self.pracLvInfo = nil
    end
    self:Clear()
    self:SetLnsr("Remove")
