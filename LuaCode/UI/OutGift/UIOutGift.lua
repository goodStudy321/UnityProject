require("UI/OutGift/OutOfGift/OutGift")

UIOutGift = UIBase:New{Name = "UIOutGift"}
local My = UIOutGift
...
function My:InitCustom()
    ...
    if GiftMgr:IsOpen() then
        self.strDic["1"] = "绝版壕礼"
        local module1 = Find(root, "outGift", des)
        self:SetModInfo(module1, OutGift, 1)
    end
...
function My:SetLnsr(func)
    OutGiftMgr.eUpAction[func](OutGiftMgr.eUpAction, self.UpAction, self)
    PropMgr.eGetAdd[func](PropMgr.eGetAdd, self.OnAdd, self)
end

-- 道具添加
function My:OnAdd(action,dic)
	if action==10436 then		
		self.dic=dic
...
function My:UpAction()
    ...
    for k,v in pairs(OutGiftMgr.actionDic) do
        local go = dic[k]
        if go then
            go:SetActive(v)
        end
