...
HotLoveCondItem = Super:New{Name = "HotLoveCondItem"};
local My = HotLoveCondItem;
...
function My:SetLuaLsnr(fn)
    --HotLoveMgr.eUpdateCond[fn](HotLoveMgr.eUpdateCond , self.UpdateCond, self);
end

function My:UpdateCond(data)
    self.cfg = data;
    self:InitDesLab();
    self:InitIcon();
    self:ChangeName();
...
function My:UpCondCount()
    local cfg = self.cfg
    local str = string.format("%s/%s", cfg.count, cfg.finishNum)
    self.lab2.text = str
    self:UpBtnState(cfg.count, cfg.finishNum)
end

--更新按钮状态
function My:UpBtnState(count, finishNum)
    if count >= finishNum then
...
function My:ChangeName()
    local num = 0
    local cfg = self.cfg
    if cfg.count >= cfg.finishNum then
        num = cfg.id + 5000
    else
        num = cfg.id + 1000
    end
...
function My:InitDesLab()
    self.lab1.text = self.cfg.content;
end

--点击按钮
function My:OnBtn()
    local O = UIMgr.Open
    local type = self.cfg.taskId;
...
function My:SetIcon(tex)
    if not LuaTool.IsNull(self.tex) then
